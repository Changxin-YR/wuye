...
> 结论基于当前源码逐层检查（页面 → 路由/API → Service → Policy/DataScope → ORM/数据库）及 131 项自动测试。Windows 全量回归已通过；部署环境 `manage.py diagnose --ai --infer` 已验证 MySQL/schema 与百炼推理链。
...
| AI 上游 | 百炼默认、Dify 兼容 | 最小披露上下文、短期委托、SSE 流式 | BailianClient/DifyClient | HTTP/SSE/tool callback/工具分片 | PASS（已完成真实百炼诊断） |
