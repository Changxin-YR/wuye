...
from models import AiAction, AuditLog, ParkingSpace, ParkingUse, User


PW = 'Fixture-only-ai-chat-parking-release-509!'
HASH = generate_password_hash(PW)


class AiChatParkingReleaseConfirmationEndToEndTests(unittest.TestCase):
    def setUp(self):
    ...
    def test_chat_only_proposes_release_and_browser_confirmation_ends_exact_active_use(self):
        space_id, use_id = self.seed_active_parking_use()
        with self.factory() as db:
            use = db.get(ParkingUse, use_id)
            self.assertEqual(use.status, 'active')
        ...
        responses = iter([
            {'id': 'release-e2e-1', 'choices': [{'message': {'content': '我先核对当前有效车位关系。'}}]},
            {'id': 'release-e2e-2', 'choices': [{'message': {'content': '已生成车位释放确认单。'}}]},
            {'id': 'release-e2e-3', 'choices': [{'message': {'content': '请由当前登录人员确认后执行释放。'}}]},
        ])
        calls = []
        ...
        actions = [
            ...
            if item.get('command') == 'parking.release'
        ]
        ...
        with self.factory() as db:
            ...
            self.assertIsNone(use.active_space)
            self.assertIsNone(use.active_vehicle)
            ...
            self.assertIsNotNone(db.scalar(select(AuditLog).where(
                ...
                AuditLog.action == 'parking.release',
