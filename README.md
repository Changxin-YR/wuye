...
线上演示：https://www.23331.cloud/wuye/

住宅物业管理平台（Flask + SQLAlchemy + MySQL + Jinja2），覆盖房屋、人员与租赁、报修工单、
投诉、访客、车辆车位、设备巡检和收费十块业务。所有写操作走同一个服务层
（`services.py` / `services_ops.py`），事务、乐观锁、幂等、回读和审计都在那里统一处理；
权限由 RBAC 加行级 DataScope 在运行时决定，页面和 AI 助手共用同一套判定。

后台内置 DeepSeek Harness 智能助手，它只通过 MCP 工具接触业务逻辑，看不到也改不到当前
登录用户权限之外的数据；高风险操作会先出确认卡片，用户点过才执行。前端是 Jinja2 服务端
渲染，没有构建步骤。26 张表，6 个 RBAC 角色，35 个权限，73 个 MCP 工具。

English: a Flask + SQLAlchemy + MySQL property-management platform covering housing,
residents, leases, work orders, complaints, visitors, parking, equipment inspection and
billing. All writes go through one service layer (transactions, optimistic locking,
idempotency, read-back, auditing); authorisation is RBAC plus row-level DataScope, shared by
the web pages and the built-in DeepSeek Harness assistant, which reaches business logic only
through MCP tools.
...
| 7 | 动态能力目录 | 无权限的工具不出现在模型工具列表；硬调仍被服务端拒绝 |
...
#     注意：reset 会 DROP 旧表，务必先备份（备份成功后才允许删）
...
> 数据范围是行级的：`community` 范围绑定到「云邻花园」这一个小区，跨小区（云邻花园二期）
...
7. 高风险确认：让助手删除房屋 / 作废账单，返回确认卡片（预览 + 过期时间），点击后才真正执行。
